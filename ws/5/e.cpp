...
void slow_f(int *a, int n, MyVector &res, int b1) {
	res.reserve(n * 4);
	ll m = 0;
	ll b1_i = 1;
	for (int i = 0; i < n; ++i) {
...
MyVector multiply(const MyVector &a, const MyVector &b) {
	if (a.size() > b.size()) {
		return multiply(b, a);
	}
	if (a.size() + b.size() <= 32) {
		return multiply_slow(a, b);
	}
	for (int i = 0; i < a.size(); ++i) {
		ca[i / K] = ll(ca[i / K].r + 0.5) + a[i] * g[i % K];
	}
	for (int i = 0; i < b.size(); ++i) {
		cb[i / K] = ll(cb[i / K].r + 0.5) + b[i] * g[i % K];
	}
	int SZ = (a.size() + K - 1) / K + (b.size() + K - 1) / K;
	int h = 1 + _log2(SZ);
	...
	MyVector res;
	res.reserve(a.size() + b.size() + 2 * K + 5);
	res.resize(a.size() + b.size() + 2 * K + 5);
	ll z = 0;
	for (int i = 0; i < SZ; ++i) {
...
MyVector add(const MyVector &a, const MyVector &b) {
	MyVector res;
