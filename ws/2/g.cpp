...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        cin >> c[i];
        if (c[i] >= 0) {
            Flow::add_oriented_edge(Flow::S, i, c[i]);
        } else {
            Flow::add_oriented_edge(i, Flow::T, -c[i]);
        }
    }
    for (int i = 0; i < n; ++i) {
        int k;
        ...
        for (int j = 0; j < k; ++j) {
            ...
            cin >> x;
            --x;
            Flow::add_oriented_edge(i, x, 2e9);
        }
    }
    Flow::scale();
    vector<int> ans;
    Flow::get_used(Flow::S, ans);
    sort(ans.begin(), ans.end());
    ll rans = 0;
    for (int i = 0; i < n; ++i) {
        bool is_in = binary_search(ans.begin(), ans.end(), i);
        if (is_in) {
            // cout << i << ' ';
            rans += c[i];
        }
